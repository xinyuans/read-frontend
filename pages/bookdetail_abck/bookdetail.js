...
Page({
  ...
  onLoad: function (options) {
    ...
    if (options.isInList === '0' || options.isInList === '1') {
      ...
      var booklist = wx.getStorageSync('booklist') || []; // Ensure booklist is an array
      var isInMyBook = Array.isArray(booklist) && booklist.some(function (item) {
    ...
    if(userInfo && userInfo.userid) { // Make sure userInfo and its userid are defined
      ...
      // Handle the case where userInfo or userid is not available
      console.log("User info is not available.");
      // Possibly prompt user to login or retrieve user information again
    }
  },
      ...
      success: function (res) {
        ...
        if (tmpData && tmpData.code == 0) {
          ...
          self.setData({
            err_page_data: {
              show: true,
              image_url: 'http://127.0.0.1:9090/myapp/err_tips/network_err.png',
